...
class ServiceTypeControllerTest extends IntegrationTestSuperclass {
  ...
  @Test
  void getAllServiceTypes_success() throws Exception {
    // Arrange
    Service service = serviceRepository.save(Fixture.serviceBuilder().build());
    ServiceType serviceType1 =
        serviceTypeRepository.save(
            ServiceType.builder().name("Test ServiceType 1").service(service).build());
    ...
    // Act
    mockMvc
        .perform(get("/v1/service-types/all"))
        // Assert
        .andExpect(status().isOk())
        .andExpect(jsonPath("$", hasSize(2)))
        .andExpect(jsonPath("$[0].id").value(serviceType1.getId()))
        .andExpect(jsonPath("$[0].name").value(serviceType1.getName()))
        .andExpect(jsonPath("$[1].id").value(serviceType2.getId()))
        .andExpect(jsonPath("$[1].name").value(serviceType2.getName()))
  ...
  public void getServiceTypeById_success() throws Exception {
    ...
    // Act
    mockMvc
        .perform(get("/v1/service-types?id=" + serviceType.getId()))
        // Assert
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(serviceType.getId()))
        .andExpect(jsonPath("$.name").value(serviceType.getName()))
        .andExpect(jsonPath("$.createdAt").exists())
        .andExpect(jsonPath("$.updatedAt").exists());
  }

  @Test
  void getServiceTypeById_whenServiceTypeNotAvailableWithGivenId_throwNotFoundException()
      throws Exception {
    // Act
    mockMvc
        .perform(get("/v1/service-types?id=99999"))
        // Assert
  ...
  @Test
  @Transactional
  void getServiceTypeByKey_success() throws Exception {
    // Arrange
    Service service = serviceRepository.save(Fixture.serviceBuilder().build());
    ServiceType serviceType = Fixture.serviceTypeBuilder().build();
    serviceType.setService(service);
    ...
    // Act
    mockMvc
        .perform(get("/v1/service-types?key=" + serviceType.getKey()))
        // Assert
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(serviceType.getId()))
        .andExpect(jsonPath("$.name").value(serviceType.getName()))
  ...
  @Test
  void getServiceTypeById_whenServiceTypeNotAvailableWithGivenKey_throwNotFoundException()
      throws Exception {
    // Act
    mockMvc
        .perform(get("/v1/service-types?key=test99999"))
        // Assert
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.message").value("ServiceType not found with key: test99999"));
  }
  ...
  void getService_whenIdAndKeyNotProvided_throwBadRequestException() throws Exception {
    // Act
    mockMvc
        .perform(get("/v1/service-types"))
        // Assert
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.message").value("Either id or key should provided."));
  }
    ...
    mockMvc
        .perform(
            post("/v1/service-types")
                ...
                .content(JsonUtil.toJson(serviceTypeDto)))
        // Assert
    ...
    mockMvc
        .perform(
            post("/v1/service-types")
                .accept(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON)
                .content(JsonUtil.toJson(ServiceTypeDto.builder().name("test").build())))
        // Assert
        .andExpect(status().isBadRequest())
  ...
  @Test
  void createServiceType_whenServiceTypeNameNotProvided_throwBadRequestException()
      throws Exception {
    // Act
    mockMvc
        .perform(
            post("/v1/service-types")
                .accept(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON)
                .content(JsonUtil.toJson(ServiceTypeDto.builder().name("").build())))
        // Assert
        .andExpect(status().isBadRequest())
  ...
  void updateServiceType_success() throws Exception {
    ...
    serviceType.setService(service);
    serviceType = serviceTypeRepository.save(serviceType);
    ServiceTypeDto updateServiceTypeDto =
    ...
    mockMvc
        .perform(
            post("/v1/service-types")
                .accept(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON)
                .content(JsonUtil.toJson(updateServiceTypeDto)))
        // Assert
        .andExpect(status().isCreated())
  ...
  @Test
  void updateServiceType_whenServiceTypeNotAvailableWithGivenId_throwNotFoundException()
      throws Exception {
    // Arrange
    Service service = serviceRepository.save(Fixture.serviceBuilder().build());
    ServiceTypeDto updateServiceTypeDto =
